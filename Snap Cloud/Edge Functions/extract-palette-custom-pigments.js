...
interface ProcessingOptions {
  numColors?: number;
  ditherMethod?: "none" | "floyd" | "atkinson";
  ditherStrength?: number;
  outputRemappedImage?: boolean;
  maxOutputSize?: number;
  customPigments?: Pigment[];
}
...
const DEFAULT_PIGMENTS: Pigment[] = [
  { name: "Titanium White", rgb: [255, 250, 244] },
  { name: "Ivory Black", rgb: [35, 31, 32] },
  { name: "Cadmium Yellow", rgb: [254, 236, 0] },
  { name: "Cadmium Orange", rgb: [237, 135, 45] },
  { name: "Cadmium Red", rgb: [227, 38, 54] },
  { name: "Alizarin Crimson", rgb: [180, 23, 60] },
  { name: "Ultramarine Blue", rgb: [25, 55, 153] },
  { name: "Phthalo Blue", rgb: [15, 43, 91] },
  { name: "Phthalo Green", rgb: [18, 53, 36] },
  { name: "Viridian", rgb: [64, 130, 109] },
  { name: "Yellow Ochre", rgb: [204, 154, 84] },
  { name: "Burnt Sienna", rgb: [138, 75, 57] },
  { name: "Burnt Umber", rgb: [115, 74, 56] },
  { name: "Raw Umber", rgb: [130, 102, 68] },
];

// ============================================================================
// SPECTRAL MIXING (Subtractive approximation)
// ============================================================================

class SpectralMixingEngine {
  private pigments: Pigment[];

  constructor(pigments: Pigment[]) {
    this.pigments = pigments;
  }

  mixTwo(rgb1: [number, number, number], rgb2: [number, number, number], t: number): [number, number, number] {
    ...
    return mixed.map(c =>
      Math.round(Math.max(0, Math.min(255, Math.pow(c, 1 / 2.2) * 255)))
    ) as [number, number, number];
  }

  mixMultiple(colors: [number, number, number][], weights: number[]): [number, number, number] {
    ...
    const pairs = colors.map((c, i) => ({ color: c, weight: norm[i] }));
  ...
  buildGamut(): GamutColor[] {
    const gamut: GamutColor[] = [];
    const n = this.pigments.length;

    console.log(`Building gamut from ${n} pigments...`);

    // Pure pigments
    for (const p of this.pigments) {
      gamut.push({ rgb: [...p.rgb], lab: rgbToLab(p.rgb[0], p.rgb[1], p.rgb[2]) });
    }

    // Adjust mixture steps based on pigment count
    // More pigments = fewer steps to keep gamut size manageable
    const steps2 = n <= 8 ? 11 : (n <= 12 ? 7 : 5);
    const steps3 = n <= 8 ? 5 : (n <= 12 ? 4 : 3);
    ...
    for (let i = 0; i < n; i++) {
      for (let j = i + 1; j < n; j++) {
        for (let s = 1; s < steps2; s++) {
          ...
          gamut.push({ rgb: [...mixed], lab: rgbToLab(mixed[0], mixed[1], mixed[2]) });
        }
      }
    }

    // Three-pigment mixtures (only if not too many pigments)
    if (n <= 16) {
      for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
          for (let k = j + 1; k < n; k++) {
            for (let si = 1; si < steps3; si++) {
              for (let sj = 1; sj < steps3 - si; sj++) {
                ...
                const mixed = this.mixMultiple(
                  [this.pigments[i].rgb, this.pigments[j].rgb, this.pigments[k].rgb],
                  [si / steps3, sj / steps3, sk / steps3]
                );
                gamut.push({ rgb: [...mixed], lab: rgbToLab(mixed[0], mixed[1], mixed[2]) });
              }
            }
    ...
    console.log(`Built gamut with ${gamut.length} colors`);
    return gamut;
  }
}
...
function deltaESimple(lab1: LabColor, lab2: LabColor): number {
...
class GamutProjector {
  ...
  project(lab: LabColor): { rgb: number[]; lab: LabColor; de: number } {
        ...
        for (let da = -radius; da <= radius; da++) {
          for (let db = -radius; db <= radius; db++) {
            ...
            for (const idx of bucket) {
              ...
              if (de < minDe) { minDe = de; bestIdx = idx; }
            }
          }
        }
      }
    ...
    if (minDe === Infinity) {
      for (let i = 0; i < this.gamut.length; i++) {
        ...
        if (de < minDe) { minDe = de; bestIdx = i; }
      }
    }

    return { rgb: [...this.gamut[bestIdx].rgb], lab: { ...this.gamut[bestIdx].lab }, de: minDe };
  }
}
...
function kmeansLab(pixels: number[][], k: number, maxIter = 30) {
  if (pixels.length === 0) return [];
  k = Math.min(k, pixels.length);

  const labPixels = pixels.map(p => ({ rgb: p, lab: rgbToLab(p[0], p[1], p[2]) }));
  const centroids: LabColor[] = [{ ...labPixels[Math.floor(Math.random() * labPixels.length)].lab }];
  ...
  for (let c = 1; c < k; c++) {
    ...
    centroids.push({ ...labPixels[bestIdx].lab });
  }
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    for (let i = 0; i < labPixels.length; i++) {
      ...
      if (assignments[i] !== nearest) { changed = true; assignments[i] = nearest; }
    }
    if (!changed) break;
    ...
    for (let i = 0; i < labPixels.length; i++) {
      ...
      s.l += labPixels[i].lab.l; s.a += labPixels[i].lab.a; s.b += labPixels[i].lab.b; s.count++;
    }
    for (let c = 0; c < centroids.length; c++) {
      if (sums[c].count > 0) {
  ...
  const clusters = centroids.map(() => ({ rgbSum: [0, 0, 0], count: 0 }));
  for (let i = 0; i < labPixels.length; i++) {
    const c = clusters[assignments[i]];
    c.rgbSum[0] += labPixels[i].rgb[0]; c.rgbSum[1] += labPixels[i].rgb[1]; c.rgbSum[2] += labPixels[i].rgb[2]; c.count++;
  }
  ...
  return clusters.filter(c => c.count > 0).map(c => {
    ...
    return { rgb, lab: rgbToLab(rgb[0], rgb[1], rgb[2]), population: c.count / labPixels.length };
...
function applyLabDithering(img: Image, palette: { rgb: number[]; lab: LabColor }[], method: string, strength: number): Image {
  ...
  for (let y = 1; y <= h; y++) {
    for (let x = 1; x <= w; x++) {
      ...
      labBuffer[idx] = lab.l; labBuffer[idx + 1] = lab.a; labBuffer[idx + 2] = lab.b;
    }
  }

  const result = new Image(w, h);
  const matrices: { [k: string]: [number, number, number][] } = {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const idx = (y * w + x) * 3;
      const current: LabColor = { l: labBuffer[idx], a: labBuffer[idx + 1], b: labBuffer[idx + 2] };

      let minDe = Infinity, closest = palette[0];
      for (const p of palette) {
        const de = deltaESimple(current, p.lab);
        if (de < minDe) { minDe = de; closest = p; }
      }
      ...
      if (matrix) {
        ...
        for (const [dx, dy, wt] of matrix) {
          ...
          if (nx >= 0 && nx < w && ny >= 0 && ny < h) {
            ...
            labBuffer[nIdx] += errL * wt; labBuffer[nIdx + 1] += errA * wt; labBuffer[nIdx + 2] += errB * wt;
          }
        }
      }
    }
...
async function extractPixels(imageData: Uint8Array, maxSize = 200) {
  const img = await decode(imageData);
  if (!(img instanceof Image)) throw new Error("Failed to decode image");
  ...
  for (let y = 1; y <= proc.height; y++) {
    for (let x = 1; x <= proc.width; x++) {
      ...
      if ((px & 0xff) > 0) pixels.push([(px >> 24) & 0xff, (px >> 16) & 0xff, (px >> 8) & 0xff]);
    }
  }
  return { width: proc.width, height: proc.height, pixels };
}

async function loadImageForRemapping(imageData: Uint8Array, maxSize = 500): Promise<Image> {
  const img = await decode(imageData);
  if (!(img instanceof Image)) throw new Error("Failed to decode image");
  const scale = Math.min(1, maxSize / Math.max(img.width, img.height));
...
// ============================================================================
// VALIDATE CUSTOM PIGMENTS
// ============================================================================

function validatePigments(pigments: any[]): Pigment[] {
  const valid: Pigment[] = [];

  for (const p of pigments) {
    if (!p || typeof p !== 'object') continue;
    if (!p.name || typeof p.name !== 'string') continue;
    if (!Array.isArray(p.rgb) || p.rgb.length !== 3) continue;

    const rgb = p.rgb.map((c: any) => {
      const n = Number(c);
      return Math.max(0, Math.min(255, Math.round(isNaN(n) ? 128 : n)));
    });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      const { imageUrl, imageBase64, customPigments, ...opts } = body;
      options = opts;

      // Handle custom pigments
      if (customPigments && Array.isArray(customPigments)) {
        options.customPigments = validatePigments(customPigments);
        console.log(`Received ${options.customPigments.length} custom pigments`);
      }
      ...
      options = {
        ...
        ditherMethod: (formData.get("ditherMethod")?.toString() || "floyd") as "none" | "floyd" | "atkinson",
    ...
    // Use custom pigments if provided and valid, otherwise defaults
    const pigments: Pigment[] = (options.customPigments && options.customPigments.length >= 2)
      ? options.customPigments
      : DEFAULT_PIGMENTS;

    console.log(`Using ${pigments.length} pigments: ${pigments.map(p => p.name).join(', ')}`);
    ...
    if (sampled.length > 10000) {
      sampled = Array.from({ length: 10000 }, () => pixels[Math.floor(Math.random() * pixels.length)]);
    }

    console.log("K-means clustering...");
    const extracted = kmeansLab(sampled, numColors);

    console.log("Building paint gamut...");
    const mixer = new SpectralMixingEngine(pigments);
    const gamut = mixer.buildGamut();

    console.log("Projecting colors...");
    const projector = new GamutProjector(gamut);
    const projected = extracted.map(c => {
      const p = projector.project(c.lab);
      return {
        rgb: p.rgb, hex: rgbToHex(p.rgb[0], p.rgb[1], p.rgb[2]), lab: p.lab,
        population: c.population, originalRgb: c.rgb, originalLab: c.lab, de: p.de,
      };
    });
    ...
    let remappedImageBase64: string | undefined;
    let remappedWidth: number | undefined;
    let remappedHeight: number | undefined;

    if (outputRemappedImage) {
      console.log("Remapping image...");
      const fullImg = await loadImageForRemapping(imageData, maxOutputSize);
      const remapped = applyLabDithering(fullImg, projected.map(c => ({ rgb: c.rgb, lab: c.lab })), ditherMethod, ditherStrength);
    ...
    return new Response(JSON.stringify({
      ...
      extractedPalette: extracted.map(c => ({
        rgb: c.rgb, hex: rgbToHex(c.rgb[0], c.rgb[1], c.rgb[2]),
        lab: { l: Math.round(c.lab.l * 100) / 100, a: Math.round(c.lab.a * 100) / 100, b: Math.round(c.lab.b * 100) / 100 },
      ...
      projectedPalette: projected.map(c => ({
        ...
        lab: { l: Math.round(c.lab.l * 100) / 100, a: Math.round(c.lab.a * 100) / 100, b: Math.round(c.lab.b * 100) / 100 },
        population: Math.round(c.population * 10000) / 100,
        originalRgb: c.originalRgb, originalHex: rgbToHex(c.originalRgb[0], c.originalRgb[1], c.originalRgb[2]),
        de: Math.round(c.de * 100) / 100,
      })),
      gamutSize: gamut.length,
      pigments: pigments.map(p => ({ name: p.name, rgb: [...p.rgb] })),
      statistics: stats,
      remappedImageBase64,
      remappedWidth,
      remappedHeight,
