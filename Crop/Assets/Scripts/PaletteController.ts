...
export class PaletteController extends BaseScriptComponent {
    ...
    @input
    @hint("Optional IDs for each prefab (comma-separated). If empty, indices will be used.")
    itemIds: string = "";
    
    @input
    @hint("Path to the colored square within item hierarchy (e.g., 'Colored Square')")
    ...
    private items: Map<string, { sceneObject: SceneObject; button: BaseButton; index: number; coloredSquare: SceneObject | null; coloredSquareMaterial: Material | null; color: vec4 }> = new Map();
    private itemList: { id: string; sceneObject: SceneObject; button: BaseButton; coloredSquare: SceneObject | null; coloredSquareMaterial: Material | null; color: vec4 }[] = [];
    private activeItemId: string | null = null;
    private isUpdatingSelection: boolean = false;
    
    private onSelectionChangedEvent: Event<PaletteSelectionEvent> = new Event<PaletteSelectionEvent>();
    public readonly onSelectionChanged: PublicApi<PaletteSelectionEvent> = this.onSelectionChangedEvent.publicApi();
    ...
    initialize(): void {
        ...
        if (this.defaultSelectedIndex >= 0 && this.defaultSelectedIndex < this.itemList.length) {
    ...
    private instantiateItems(): void {
        const ids = this.itemIds.split(",").map(id => id.trim()).filter(id => id.length > 0);
        ...
        for (let i = 0; i < this.itemPrefabs.length; i++) {
            ...
            if (coloredSquare) {
                const renderMesh = coloredSquare.getComponent("Component.RenderMeshVisual") as RenderMeshVisual;
                ...
                print(`PaletteController: No colored square found at path '${this.coloredSquarePath}' for prefab ${i}`);
            }
            ...
            const itemData = { sceneObject, button, index: this.itemList.length, coloredSquare, coloredSquareMaterial, color: defaultColor };
            this.items.set(id, itemData);
            this.itemList.push({ id, sceneObject, button, coloredSquare, coloredSquareMaterial, color: defaultColor });
        }
        
        print(`PaletteController: Instantiated ${this.itemList.length} items`);
    }
    
    private findChildByPath(sceneObject: SceneObject, path: string): SceneObject | null {
    ...
    private findButtonComponent(sceneObject: SceneObject): BaseButton | null {
        ...
        let button = sceneObject.getComponent("Component.ScriptComponent") as BaseButton;
    ...
    private calculateBoundingBox(sceneObject: SceneObject): { min: vec3; max: vec3; size: vec3; center: vec3 } {
        const defaultBox = {
            min: vec3.zero(),
            max: vec3.zero(),
            size: vec3.zero(),
    ...
    private findMeshVisuals(sceneObject: SceneObject): RenderMeshVisual[] {
        ...
        const meshVisual = sceneObject.getComponent("Component.RenderMeshVisual") as RenderMeshVisual;
        if (meshVisual) {
