@component
export class ColorMixTest extends BaseScriptComponent {
    ...
    onAwake(): void {
        ...
        this.mainPass.numPigments = ColorMixTest.NUM_PIGMENTS;
        this.mainPass.texWidth = ColorMixTest.NUM_PIGMENTS;
        
        this.rebuildPixels();
        
        this.createEvent("UpdateEvent").bind(() => this.rebuildPixels());
        
        print("ColorMixTest initialized");
    }
    
    private rebuildPixels(): void {
        const pixels = new Uint8Array(ColorMixTest.NUM_PIGMENTS * 4);
        ...
        for (let i = 0; i < ColorMixTest.NUM_PIGMENTS; i++) {
            const idx = i * 4;
        ...
        (this.proceduralTexture.control as ProceduralTextureProvider).setPixels(
            0, 0, ColorMixTest.NUM_PIGMENTS, 1, pixels
        );
    }
}